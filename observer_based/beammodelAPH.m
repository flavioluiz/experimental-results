...
classdef beammodelAPH
    ...
    methods
        function beam = beammodelAPH(nben,ntor, damp, params)            
                beam.NUMBEN = nben;
                beam.NUMTOR = ntor;
                beam.damp = damp;
        ...
        function A = getA(beam)            
            ...
            Rigid = diag([beam.omeganB beam.omeganTo]);
            A = [-Damp -Rigid; Rigid zeros(size(Damp))];
        end      
        ...
        function Bv = getBpiezo(beam,pos) %pos = +1,-1
            ...
            Bt = beam.Bvt;
            BB = [By pos*Bt];
            Rigid = [beam.omeganB beam.omeganTo];
            Bv = [BB./Rigid, BB*0]';            
        end
        function Cp = getCpiezo(beam,pos) %pos = +1,-1
            % calculate C matrix (piezoelectric effect):
              % something equivalent to current
            By = beam.Bvb;
            Bt = beam.Bvt;
            BB = [By pos*Bt];
            Rigid = [beam.omeganB beam.omeganTo];
