clear all;
global Tfinal input output times T AMP Kd control_input T_STOP;
%% launch daq session
...
AMP = 100*kamp.^T.*cos(PHASE_1+(2*pi*f0.*(k.^T-1)/log(k)) );
...
observer
obsv = ss(A-L*Cv, [Bvoltage,L], Cp, 0);
Kd = c2d(obsv, Ts);
Fs = 1/Ts;

%% EXCITATION WITH CONTROL
KON = 1;
GAIN = 100000;
diff_phase = 0;
PHASE_1 = pi/2;
...
AMP = 100*kamp.^T.*cos(PHASE_1+(2*pi*f0.*(k.^T-1)/log(k)) );
run_save_test('symexc_1_KON_100000_co');
%run_save_test('symexc_2_KON_100000_co');
...
PHASE_2 = pi/2 + diff_phase*pi/180;
AMP = 100*kamp.^T.*cos(PHASE_1+(2*pi*f0.*(k.^T-1)/log(k)) );
run_save_test('symexc_1_KON_1000000_co');
%run_save_test('symexc_2_KON_1000000_co');
...
PHASE_2 = pi/2 + diff_phase*pi/180;
AMP = 100*kamp.^T.*cos(PHASE_1+(2*pi*f0.*(k.^T-1)/log(k)) );
run_save_test('symexc_1_co');
%run_save_test('symexc_2_co');
...
PHASE_2 = pi/2 + diff_phase*pi/180;
AMP = 100*kamp.^T.*cos(PHASE_1+(2*pi*f0.*(k.^T-1)/log(k)) );
run_save_test('symexc_1_KON_20000_co');
%run_save_test('symexc_2_KON_20000_co');
